package com.sysout.buy_zone_api.security;

import com.sysout.buy_zone_api.Util.AuthExceptionMessages;
...
public class CustomPasswordAuthenticationProvider implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		try {
			...
			throw new OAuth2AuthenticationException(AuthExceptionMessages.INVALID_CREDENTIALS);
		}

		if (!user.isEnabled()) {
			throw new OAuth2AuthenticationException(AuthExceptionMessages.USER_DISABLED);
		}

		if (!user.isAccountNonLocked()) {
			throw new OAuth2AuthenticationException(AuthExceptionMessages.USER_LOCKED);
		}

		if (!user.isAccountNonExpired()) {
			throw new OAuth2AuthenticationException(AuthExceptionMessages.ACCOUNT_EXPIRED);
		}

		if (!user.isCredentialsNonExpired()) {
			throw new OAuth2AuthenticationException(AuthExceptionMessages.CREDENTIALS_EXPIRED);
		}
		...
		if (user instanceof User customUser && customUser.isMfaEnabled()) {
			System.out.println("Chamando serviço de MFA");
		}
